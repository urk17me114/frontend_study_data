...
from login.models import Login, Student, Marks, StudentReg, TeacherReg, TimetableEntry, ClassSection, Subject, TeacherAvailability, TimeSlot,StudentReg1,Teacher
...
def add_marks_step2(request):
    ...
    # Extract class level number from class_section.name to match Student.classlevel
    #level = ''.join(filter(str.isdigit, class_section.name))
    level = class_section.name

    # Filter students by classlevel
...
def view_marks(request):
    ...
    try:
        ...
        return redirect('view_marks')  # or show a custom error page
