...
class TeacherProfileForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'firstname': forms.TextInput(attrs={'readonly': 'readonly', 'style': 'background-color:#e9ecef;'}),
            'lastname': forms.TextInput(attrs={'readonly': 'readonly', 'style': 'background-color:#e9ecef;'}),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for field, value in cleaned_data.items():
                ...
                if hasattr(student_model, field):
...
def teacher_profile(request):
    # IDOR vulnerability introduced here: attacker can supply ?username=KP0001
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('teacher_profile')  # Will reload logged-in user's profile
    else:
        initial_data = {
...
def download_teacher_document(request):
    ...
    try:
        ...
        return FileResponse(open(document_path, 'rb'), as_attachment=True, filename=os.path.basename(document_path))

    except Exception as e:
        print(f"Download error: {e}")  # For debug
