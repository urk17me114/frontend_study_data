...
def student_my_submissions(request):
    ...
    subs = AssignmentSubmission.objects.filter(student=student).select_related('question', 'question__subject', 'question__class_section').order_by('-submitted_at')

    return render(request, 'login/student_my_submissions.html', {
        'submissions': subs
    })
