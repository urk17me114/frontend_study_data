...
class StudentAssignment(models.Model):
    ...
    # If your student model is StudentReg, change to 'StudentReg'

    subject = models.ForeignKey('Subject', on_delete=models.PROTECT, related_name='student_assignments')
    class_section = models.ForeignKey('ClassSection', on_delete=models.PROTECT, related_name='student_assignments')
    ...
    def __str__(self):
        return f"{self.title} — {self.student} ({self.subject})"
    

class AssignmentQuestion(models.Model):
    title = models.CharField(max_length=200)
    description = models.TextField()
    subject = models.ForeignKey('login.Subject', on_delete=models.CASCADE, related_name='questions')
    class_section = models.ForeignKey('login.ClassSection', on_delete=models.CASCADE, related_name='questions')
    attachment = models.FileField(upload_to='questions/', blank=True, null=True)
    due_date = models.DateTimeField(blank=True, null=True)

    # who created
    teacher_username = models.CharField(max_length=150)  # aligns with TeacherAvailability.username
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.title} ({self.subject} - {self.class_section})"


class AssignmentSubmission(models.Model):
    """
    Student submits AS A RESPONSE TO a specific question.
    """
    STATUS_CHOICES = [
        ('submitted', 'Submitted'),
        ('seen', 'Seen'),
    ...
    question = models.ForeignKey(AssignmentQuestion, on_delete=models.CASCADE, related_name='submissions')
    student = models.ForeignKey('login.Student', on_delete=models.CASCADE, related_name='assignment_submissions')
...
# Pinboard announcement model
class Pinboard(models.Model):
    id = models.AutoField(primary_key=True)
    title = models.CharField(max_length=200)
    content = models.TextField()  # XSS vulnerable
    created_at = models.DateTimeField(default=timezone.now)
    created_by = models.CharField(max_length=100)  # Admin username

    def __str__(self):
        return self.title

# Comments under announcements
class PinboardComment(models.Model):
    id = models.AutoField(primary_key=True)
    announcement = models.ForeignKey(Pinboard, on_delete=models.CASCADE, related_name="comments", null=True, blank=True)
    user = models.CharField(max_length=100, null=True, blank=True)  
    comment = models.TextField(null=True, blank=True)  # XSS vulnerable
    created_at = models.DateTimeField(default=timezone.now)
