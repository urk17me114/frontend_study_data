...
Functionality:
    ...
    Verifies the security question answer for the identified user.
    If valid, sets a session flag.
    Allows the user to reset their password after successful verification.
    Validates and confirms new password entries.

Vulnerabilities:
    Skipping the oder of work flow for the users without a security question.
...
# -------------------------------
# Forgot Password Step 2: Verify Security Question
# -------------------------------
@never_cache
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if answer.strip().lower() == user.security_answer.strip().lower():
                ...
                request.session.flush()
                messages.warning(request, "Wrong answer to security question")
                return redirect('index')
    else:
